...
int main(int argc, char** argv)
{
  ...
  do
  {
    ...
    else
    {
      ...
      data_space.resize(max_ybin);
      data_space_starttimes.resize(max_ybin);
      data_space_stoptimes.resize(max_ybin);
      for(int o = 0; o < max_ybin; o++)
      {
        data_space[o].resize(max_xbin);
        data_space_starttimes[o].resize(max_xbin);
        data_space_stoptimes[o].resize(max_xbin);
      }

      for(int s = 0; s < pos_ybin.size(); s++)
      {
        ...
        if(data_space_starttimes[pos_ybin[s]][pos_xbin[s]] == 0)
        {

          data_space_starttimes[pos_ybin[s]][pos_xbin[s]] = comp_time;
        }
        data_space_stoptimes[pos_ybin[s]][pos_xbin[s]] = comp_time;

        data_space[pos_ybin[s]][pos_xbin[s]] = data_space[pos_ybin[s]][pos_xbin[s]] + 1;
        if(neut[s] == 0)
        {
          data_space_gamma[pos_ybin[s]][pos_xbin[s]] = data_space_gamma[pos_ybin[s]][pos_xbin[s]] + 1;
        }
      ...
      for(int s = 0; s < pos_ybin.size(); s++)
      {
        data_space[pos_ybin[s]][pos_xbin[s]] = data_space[pos_ybin[s]][pos_xbin[s]] /
          data_space_stoptimes[pos_ybin[s]][pos_xbin[s]] - data_space_starttimes[pos_ybin[s]][pos_xbin[s]];
      }
    ...
    {
      for(int spacey = 0; spacey < data_space.size(); spacey++)
      {
        for(int spacex = 0; spacex < data_space[0].size(); spacex++)
        {
          source_space[spacey][spacex] = space_sim[spacey][spacex];
        }
      }
    ...
    */
    ...
    conn = PQconnectdb("dbname=det_db user=postgres password=postgres hostaddr=10.0.100.1 port=5432");
    ...
    if(sim == 0)
    {
      ...
      res = PQexec(conn, "create table if not exists heatmap (ind SERIAL, counts integer NOT NULL, neutron smallint NOT NULL, gamma smallint NOT NULL, xbin integer NOT NULL, ybin integer NOT NULL)");
    ...
    //
    res = PQexec(conn, "create table if not exists spect_res (ind SERIAL, iso character varying(32) NOT NULL, intensity integer NOT NULL)");
    ...
    //
    res = PQexec(conn, "create table if not exists spatial_res (ind SERIAL, xbin integer NOT NULL, ybin integer NOT NULL, confidence real NOT NULL)");
